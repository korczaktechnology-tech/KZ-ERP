...
const lookupMap:Record<string,string>={
  'Lista de preços (ID)':'/price-lists?limit=1000&offset=0',
  'Produto (ID)':'/products?limit=1000&offset=0',
  'Parceiro (ID)':'/parties?limit=1000&offset=0',
  'Armazém (ID)':'/warehouses?limit=1000&offset=0',
...
async function addLookup(input:HTMLInputElement,label:string){
  const path=lookupMap[label];
  if(!path||input.dataset.kzLookup==='ready')return;
  ...
  try{
    ...
    input.title='Selecione uma opção ou pesquise pelo nome/código.';
...
function addDownloads(root:ParentNode){
  ...
  card?.querySelectorAll<HTMLTableRowElement>('tbody tr').forEach(row=>{
    ...
    button.type='button';button.textContent='Baixar';button.className='kz-f2-download';button.title='Baixar arquivo';
    button.addEventListener('click',async()=>{
      ...
      try{
        ...
        const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=row.querySelector('.f2-pro-primary')?.textContent?.trim()||`anexo-${id}`;a.click();URL.revokeObjectURL(url);
