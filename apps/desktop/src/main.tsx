...
type Party = { _id: string; code: string; name: string; active: boolean };
...
async function apiWithSession<T>(path: string, session: Session, options: RequestInit = {}, onRefresh?: (next: Session) => void): Promise<T> {
  ...
  try {
    ...
    const next = { ...session, ...refreshed }; saveSession(next); onRefresh?.(next);
    response = await requestRaw(path, options, next.accessToken); return parseResponse<T>(response);
  } catch { clearSession(); throw new Error('Sessão expirada. Entre novamente.'); }
}

function WindowControls() {
  const win = getCurrentWindow();
  return <div className="window-controls" data-tauri-drag-region><button aria-label="Minimizar" onClick={() => void win.minimize()}>−</button><button aria-label="Maximizar ou restaurar" onClick={() => void win.toggleMaximize()}>□</button><button className="close-window" aria-label="Fechar" onClick={() => void win.close()}>×</button></div>;
}
function Titlebar({ status }: { status: string }) {
  const win = getCurrentWindow();
  return <div className="titlebar" data-tauri-drag-region onDoubleClick={() => void win.toggleMaximize()}><div className="titlebar-brand"><span className="brand-mark mini">KZ</span><strong>KORCZAK ERP</strong><span className="titlebar-status">{status}</span></div><WindowControls /></div>;
}
...
function App() {
  ...
  const checkConnection = React.useCallback(async () => {
    ...
    try { await api<{ status: string }>('/health'); const info = await api<SystemInfo>('/api/v1/system'); setSystem(info); setApiStatus('online'); }
    catch (error) { setApiStatus('offline'); setHealthError(error instanceof Error ? error.message : 'Falha de conexão'); }
  }, []);
  ...
  const hydrateSession = React.useCallback(async (current: Session) => {
    try {
      const [identity, companyData] = await Promise.all([apiWithSession<Me>('/api/v1/core/me', current, {}, setSession), apiWithSession<{ company: Company }>('/api/v1/core/company', current, {}, setSession)]);
      setMe(identity); setCompany(companyData.company);
    } catch (error) { clearSession(); setSession(null); setMe(null); setCompany(null); setToast(error instanceof Error ? error.message : 'Sessão inválida.'); }
  }, []);
...
{view === 'overview' && <Dashboard apiStatus={apiStatus} session={session} company={company} system={system} onLogin={() => setLoginOpen(true)} onNavigate={setView}/>} 
{view === 'cadastros' && (session ? <F2MasterData session={session} onToast={setToast}/> : <AuthRequired onLogin={() => setLoginOpen(true)}/>)}
...
</main></div>{loginOpen && <Login onClose={() => setLoginOpen(false)} onSuccess={next => { saveSession(next); setSession(next); setLoginOpen(false); setToast('Login realizado.'); }}/>} {toast && <div className="toast" onAnimationEnd={() => setToast('')}>{toast}</div>}</div>;
}
...
function SalesPage({ session, onToast }: { session: Session | null; onToast: (s: string) => void }) { if (!session) return <AuthRequired onLogin={() => onToast('Faça login pelo botão Entrar no topo.')}/>; return <SalesContent session={session} onToast={onToast}/>; }
function SalesContent({ session, onToast }: { session: Session; onToast: (s: string) => void }) {
  ...
  const load=React.useCallback(async()=>{setState('loading');try{const [o,c,p]=await Promise.all([apiWithSession<Paginated<Order>>('/api/v1/sales/orders?limit=100&offset=0',session),apiWithSession<Paginated<Party>>('/api/v1/master-data/customers?limit=100&offset=0',session),apiWithSession<Paginated<Product>>('/api/v1/master-data/products?limit=100&offset=0',session)]);setOrders(o.items);setCustomers(c.items);setProducts(p.items);setState('ready')}catch{setState('error')}},[session]); React.useEffect(()=>{void load()},[load]);
...
function OrderForm({session,customers,products,onClose,onCreated}:{session:Session;customers:Party[];products:Product[];onClose:()=>void;onCreated:()=>Promise<void>}){const[number,setNumber]=React.useState('');const[customerId,setCustomerId]=React.useState(customers[0]?._id??'');const[productId,setProductId]=React.useState(products[0]?._id??'');const[quantity,setQuantity]=React.useState('1');const[unitPrice,setUnitPrice]=React.useState(String(products[0]?.price??0));const[error,setError]=React.useState('');const[busy,setBusy]=React.useState(false);function chooseProduct(id:string){setProductId(id);setUnitPrice(String(products.find(p=>p._id===id)?.price??0))}async function submit(e:React.FormEvent){e.preventDefault();setBusy(true);setError('');try{await apiWithSession('/api/v1/sales/orders',session,{method:'POST',body:JSON.stringify({number,customerId,lines:[{productId,quantity:Number(quantity),unitPrice:Number(unitPrice)}]})});await onCreated()}catch(e){setError(e instanceof Error?e.message:'Não foi possível criar o pedido.')}finally{setBusy(false)}}return <Modal title="Criar pedido" onClose={onClose}><form onSubmit={submit}><label>Número<input value={number} onChange={e=>setNumber(e.target.value)} required/></label><label>Cliente<select value={customerId} onChange={e=>setCustomerId(e.target.value)}>{customers.map(c=><option key={c._id} value={c._id}>{c.name}</option>)}</select></label><label>Produto<select value={productId} onChange={e=>chooseProduct(e.target.value)}>{products.map(p=><option key={p._id} value={p._id}>{p.name} · R$ {p.price.toFixed(2)}</option>)}</select></label><div className="form-row"><label>Quantidade<input type="number" min="0.000001" step="any" value={quantity} onChange={e=>setQuantity(e.target.value)} required/></label><label>Preço unitário<input type="number" min="0" step="0.01" value={unitPrice} onChange={e=>setUnitPrice(e.target.value)} required/></label></div>{error&&<div className="form-error">{error}</div>}<button className="primary" disabled={busy}>{busy?'Criando…':'Criar pedido'}</button></form></Modal>;}
