...
async fn download_to_file(app: &AppHandle, url: &str, path: &Path, expected_sha256: &str, phase: &str) -> Result<(), String> { trusted_url(url)?; let client = reqwest::Client::builder().user_agent("KORCZAK-ERP-Updater/2.0").timeout(DOWNLOAD_TIMEOUT).build().map_err(|e| format!("create downloader: {e}"))?; let response = client.get(url).send().await.map_err(|e| format!("download: {e}"))?; if !response.status().is_success() { return Err(format!("download HTTP {}", response.status())); } let total = response.content_length().unwrap_or(0); if total > MAX_DOWNLOAD_BYTES { return Err("download exceeds maximum allowed size".into()); } let mut stream = response.bytes_stream(); let mut file = tokio::fs::File::create(path).await.map_err(|e| format!("create update file: {e}"))?; let mut hasher = Sha256::new(); let mut downloaded = 0_u64; while let Some(chunk) = futures_util::StreamExt::next(&mut stream).await { let chunk = chunk.map_err(|e| format!("read download: {e}"))?; downloaded = downloaded.saturating_add(chunk.len() as u64); if downloaded > MAX_DOWNLOAD_BYTES { return Err("download exceeds maximum allowed size".into()); } hasher.update(&chunk); file.write_all(&chunk).await.map_err(|e| format!("write update: {e}"))?; let percent = if total > 0 { ((downloaded.saturating_mul(100)) / total).min(100) as u8 } else { 0 }; let _ = app.emit("update-progress", serde_json::json!({"phase":phase,"downloaded":downloaded,"total":total,"percent":percent})); } file.flush().await.map_err(|e| format!("flush update: {e}"))?; if downloaded < 1024 * 1024 { return Err("download is unexpectedly small".into()); } let actual = format!("{:x}", hasher.finalize()); if actual != expected_sha256.to_ascii_lowercase() { return Err("update checksum mismatch".into()); } Ok(()) }
