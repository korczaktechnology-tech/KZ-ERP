...
async function githubFetch(input: string, init: RequestInit = {}) { const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS); try { return await fetch(input, { ...init, signal: controller.signal }); } finally { clearTimeout(timer); } }
...
async function signedManifest(release: Release) {
  ...
  const response = await githubFetch(`${GH}/repos/${REPO}/releases/assets/${sig.id}`, { headers: headers(true), redirect: 'follow' }); if (!response.ok) throw new Error('UPDATE_SIGNATURE_LOOKUP_FAILED');
  const manifest = await response.json() as { version?: string; assetId?: number; assetName?: string; sha256?: string; signature?: string }; const releaseVersion = version(release.tag_name);
  if (!releaseVersion || manifest.version !== releaseVersion || manifest.assetId !== deb.id || manifest.assetName !== deb.name || !validSha256(manifest.sha256) || !validSignature(manifest.signature)) throw new Error('UPDATE_SIGNATURE_INVALID');
  const digest = await checksumFromAsset(release, deb); if (digest && digest !== manifest.sha256.toLowerCase()) throw new Error('UPDATE_SIGNATURE_DIGEST_MISMATCH');
...
router.get('/latest', async (_req, res) => {
  ...
  try { const release = await latestRelease(); const deb = debAsset(release); if (!deb?.id || !deb.name) return res.status(404).json({ error: 'RELEASE_ASSET_NOT_FOUND' }); const manifest = await signedManifest(release); res.json({ version: manifest.version, tag: release.tag_name, name: release.name, notes: release.body ?? '', assetId: deb.id, assetName: deb.name, sha256: manifest.sha256, signature: manifest.signature }); }
  catch (error) { console.error(error); const message = error instanceof Error ? error.message : ''; const status = message.startsWith('GITHUB_RELEASE_LOOKUP_FAILED:') ? Number(message.split(':')[1]) : 502; res.status(status >= 400 && status < 600 ? status : 502).json({ error: message.startsWith('GITHUB_RELEASE_LOOKUP_FAILED:') ? 'GITHUB_RELEASE_LOOKUP_FAILED' : message || 'GITHUB_RELEASE_LOOKUP_FAILED' }); }
});

router.get('/previous', async (req, res) => {
  if (!token) return res.status(503).json({ error: 'UPDATE_SERVICE_NOT_CONFIGURED' }); const before = typeof req.query.before === 'string' ? req.query.before : ''; const current = version(before); if (!current) return res.status(400).json({ error: 'INVALID_VERSION' });
  try {
