...
const requiredRoutes=['contacts','warehouse-locations','classifications','attachments','relationships','search','integrity','export/:entity','import/:entity'];

test('F2 exposes every planned extension area',()=>{
  assert.match(source,/crud\('category',\s*'categories'\)/);
  assert.match(source,/crud\('brand',\s*'brands'\)/);
  for(const route of requiredRoutes) assert.match(source,new RegExp(route.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
});
