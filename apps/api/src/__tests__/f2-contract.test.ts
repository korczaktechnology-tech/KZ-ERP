...
test('F2 exposes every planned extension area',()=>{
  assert.match(source,/crud\('category','categories'\)/);
  assert.match(source,/crud\('brand','brands'\)/);
  for(const route of requiredRoutes.slice(2)) assert.match(source,new RegExp(route.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
});
