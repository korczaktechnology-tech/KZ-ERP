...
async function audit(db: Db, companyId: string, actorUserId: string, action: string, resource: string, resourceId: string | undefined, metadata?: Record<string, unknown>) {
  await db.collection<{ _id: string; companyId: string; actorUserId: string; action: string; resource: string; resourceId?: string; metadata?: Record<string, unknown>; createdAt: Date }>('audit_logs').insertOne({ _id: id(), companyId, actorUserId, action, resource, resourceId, metadata, createdAt: now() });
}
