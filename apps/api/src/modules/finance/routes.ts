...
function serialize(e: FinancialEntry) { const { companyId: _companyId, idempotencyKey: _idempotencyKey, operationHash: _operationHash, ...safe } = e; return { ...safe, amount: e.amount.toString() }; }
function cents(value: Decimal128): bigint { const raw = value.toString(); const [whole = '0', fraction = ''] = raw.split('.'); return BigInt(whole) * 100n + BigInt((fraction + '00').slice(0, 2)); }
function centsString(value: bigint): string { const sign = value < 0n ? '-' : ''; const absolute = value < 0n ? -value : value; return `${sign}${absolute / 100n}.${(absolute % 100n).toString().padStart(2, '0')}`; }
...
export function financeRouter(db: Db): Router {
  ...
  router.patch('/entries/:id', async (req, res, next) => { try { if (!canWrite(res)) { fail(res, 403, 'FORBIDDEN'); return; } const a = actor(res), eid = id.parse(req.params.id), input = updateSchema.parse(req.body); const set: { description?: string; amount?: Decimal128; dueDate?: Date; reference?: string; updatedAt: Date } = { updatedAt: new Date() }; if (input.description !== undefined) set.description = input.description; if (input.amount !== undefined) set.amount = Decimal128.fromString(input.amount); if (input.dueDate !== undefined) set.dueDate = new Date(input.dueDate); if (input.reference !== undefined) set.reference = input.reference; const result = await entries.updateOne(a.companyId, { _id: eid, status: 'open' }, { $set: set }); if (result.matchedCount === 0) { const current = await entries.findOne(a.companyId, { _id: eid }); if (!current) { fail(res, 404, 'NOT_FOUND', 'Financial entry not found'); return; } fail(res, 409, 'CONFLICT', 'Only open entries can be edited'); return; } const updated = await entries.findOne(a.companyId, { _id: eid }); if (!updated) { fail(res, 500, 'INTERNAL_ERROR', 'Financial entry disappeared after update'); return; } await audit(a, 'finance.entry.update', eid); ok(res, { entry: serialize(updated) }); } catch (e) { next(e); } });

  async function transition(req: Parameters<typeof requireAuth>[0], res: Parameters<typeof requireAuth>[1], next: Parameters<typeof requireAuth>[2], action: 'pay' | 'cancel') { try { if (!canWrite(res)) { fail(res, 403, 'FORBIDDEN'); return; } const a = actor(res), eid = id.parse(req.params.id); const update = action === 'pay' ? { $set: { status: 'paid' as const, paidAt: new Date(), updatedAt: new Date() } } : { $set: { status: 'cancelled' as const, updatedAt: new Date() } }; const result = await entries.updateOne(a.companyId, { _id: eid, status: 'open' }, update); if (result.matchedCount === 0) { const current = await entries.findOne(a.companyId, { _id: eid }); if (!current) { fail(res, 404, 'NOT_FOUND', 'Financial entry not found'); return; } fail(res, 409, 'CONFLICT', 'Only open entries can change status'); return; } const updated = await entries.findOne(a.companyId, { _id: eid }); if (!updated) { fail(res, 500, 'INTERNAL_ERROR', 'Financial entry disappeared after transition'); return; } await audit(a, `finance.entry.${action}`, eid); ok(res, { entry: serialize(updated) }); } catch (e) { next(e); } }
  ...
  router.get('/summary', async (req, res, next) => { try { if (!canRead(res)) { fail(res, 403, 'FORBIDDEN'); return; } const a = actor(res), rows = await entries.find(a.companyId, { status: 'open' }).toArray(), now = new Date(); let openReceivable = 0n, openPayable = 0n, overdueReceivable = 0n, overduePayable = 0n; for (const row of rows) { const value = cents(row.amount); if (row.type === 'receivable') { openReceivable += value; if (row.dueDate < now) overdueReceivable += value; } else { openPayable += value; if (row.dueDate < now) overduePayable += value; } } ok(res, { summary: { openReceivable: centsString(openReceivable), openPayable: centsString(openPayable), overdueReceivable: centsString(overdueReceivable), overduePayable: centsString(overduePayable), openCount: rows.length } }); } catch (e) { next(e); } });
  return router;
}
