...
import { f2SearchRouter } from './f2-search-router.js';
/** Composition root for the complete F2 surface. Route order is intentional: policy guards and complete search precede legacy routes. */
export function f2ModuleRouter(db:Db):Router{const router=Router();router.use(f2AttachmentGuardRouter(db));router.use(f2DataHealthRouter(db));router.use(f2SemanticGuardRouter(db));router.use(f2CoreExtensionsRouter(db));router.use(f2RelationshipUpdateRouter(db));router.use(f2SearchRouter(db));router.use(f2Router(db));router.use(f2ImportRouter(db));router.use(f2IoRouter(db));return router;}
