...
export function f2IoRouter(db: Db): Router {
  ...
  router.post('/attachments/upload', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const entityType = req.header('x-f2-entity-type')?.trim(); const entityId = req.header('x-f2-entity-id')?.trim(); const fileName = req.header('x-f2-file-name')?.trim(); const mimeType = req.header('x-f2-mime-type')?.trim() || 'application/octet-stream'; const size = Number(req.header('content-length') ?? 0);
      ...
      const referenceError = await validateF2Reference(db, a.companyId, entityType, entityId, 'Attachment target'); if (referenceError) return fail(res, 422, 'VALIDATION_ERROR', referenceError);
      const id = randomUUID(); const upload = bucket.openUploadStreamWithId(id as any, fileName, { metadata: { companyId: a.companyId, entityType, entityId, mimeType } }); let received = 0; let aborted = false;
      ...
      upload.on('error', next); upload.on('finish', async () => {
        ...
        try { const now = new Date(); const doc: Doc = { _id: id, companyId: a.companyId, entityType, entityId, fileName, mimeType, size, storageKey: `f2_attachments/${a.companyId}/${id}`, storageManaged: true, createdAt: now, updatedAt: now }; await collections.attachments.insertOne(a.companyId, doc); await audit(db, a, 'master-data.attachment.upload', 'attachment', id, { size, entityType, entityId }); ok(res, { attachment: { id, entityType, entityId, fileName, mimeType, size, storageKey: doc.storageKey, storageManaged: true, createdAt: now, updatedAt: now } }); }
  ...
  router.get('/attachments/:id/download', async (req: Request, res: Response, next: NextFunction) => {
    try { const a = actor(res); if (!hasPermission(a.role, 'master-data:read')) return fail(res, 403, 'FORBIDDEN'); const id = String(req.params.id); const doc = await collections.attachments.findOne(a.companyId, { _id: id }); if (!doc) return fail(res, 404, 'NOT_FOUND'); if (!doc.storageManaged) return fail(res, 409, 'CONFLICT', 'Attachment is externally managed'); const file = await db.collection('f2_attachments.files').findOne({ _id: id as any, 'metadata.companyId': a.companyId }, { projection: { _id: 1 } }); if (!file) return fail(res, 410, 'GONE', 'Attachment binary is missing'); res.setHeader('Content-Type', String(doc.mimeType)); res.setHeader('Content-Length', String(doc.size)); res.setHeader('Content-Disposition', `attachment; filename="${String(doc.fileName).replace(/["\\\r\n]/g, '_')}"`); const stream = bucket.openDownloadStream(id as any); stream.on('error', next); stream.pipe(res); } catch (error) { next(error); }
  });

  router.delete('/attachments/:id/file', async (req: Request, res: Response, next: NextFunction) => {
    try { const a = actor(res); if (!hasPermission(a.role, 'master-data:write')) return fail(res, 403, 'FORBIDDEN'); const id = String(req.params.id); const doc = await collections.attachments.findOne(a.companyId, { _id: id }); if (!doc) return fail(res, 404, 'NOT_FOUND'); if (!doc.storageManaged) return fail(res, 409, 'CONFLICT', 'Attachment is externally managed'); await bucket.delete(id as any).catch((error) => { if (!String(error).toLowerCase().includes('not found')) throw error; }); await collections.attachments.deleteOne(a.companyId, { _id: id }); await audit(db, a, 'master-data.attachment.delete-file', 'attachment', id); noContent(res); } catch (error) { next(error); }
  });

  router.get('/bulk/export/:entity', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const a = actor(res); if (!hasPermission(a.role, 'master-data:read')) return fail(res, 403, 'FORBIDDEN'); const entity = String(req.params.entity) as (typeof entities)[number]; if (!entities.includes(entity)) return fail(res, 404, 'NOT_FOUND', 'Unsupported master-data entity'); const docs = await collections[entity].find(a.companyId, {}).sort({ createdAt: 1 }).toArray(); await audit(db, a, `master-data.${entity}.bulk-export`, entity, randomUUID(), { count: docs.length });
      const records = docs.map((doc: Doc) => { const { _id, companyId: _companyId, ...rest } = doc; return { id: _id, ...rest }; }); ok(res, { entity, count: records.length, records });
    } catch (error) { next(error); }
  });
  return router;
}
