...
export function f2IoRouter(db: Db): Router {
  ...
  router.post('/attachments/upload', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const entityType = req.header('x-f2-entity-type')?.trim();
      const entityId = req.header('x-f2-entity-id')?.trim();
      const fileName = req.header('x-f2-file-name')?.trim();
      const mimeType = req.header('x-f2-mime-type')?.trim() || 'application/octet-stream';
      const size = Number(req.header('content-length') ?? 0);
