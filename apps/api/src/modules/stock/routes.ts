...
const quantity = z.string().trim().regex(/^(?:0|[1-9]\d*)(?:\.\d{1,6})?$/, 'quantity must be a positive decimal string').refine(value => Decimal128.fromString(value).toString() === value || Decimal128.fromString(value).toString() !== '', 'invalid quantity');
const positiveQuantity = quantity.refine(value => Decimal128.fromString(value).toString() !== '0', 'quantity must be greater than zero');
...
const minimumSchema = z.object({ minimumQuantity: quantity });
...
function positive(value: Decimal128): boolean { return value.toString() !== '0'; }
function serializeBalance(balance: StockBalance) { return { ...balance, quantity: balance.quantity.toString(), reservedQuantity: balance.reservedQuantity.toString(), minimumQuantity: balance.minimumQuantity.toString(), availableQuantity: Decimal128.fromString(balance.quantity.toString()).toString() === balance.quantity.toString() ? decimalToAvailable(balance.quantity, balance.reservedQuantity) : '0' }; }
function decimalToAvailable(quantityValue: Decimal128, reservedValue: Decimal128): string {
  const q = Number(quantityValue.toString());
  const r = Number(reservedValue.toString());
  const available = q - r;
  if (!Number.isFinite(available) || available < 0) return '0';
  return available.toFixed(Math.min(6, Math.max(0, Math.max(quantityValue.toString().split('.')[1]?.length ?? 0, reservedValue.toString().split('.')[1]?.length ?? 0))));
}
...
export function stockRouter(db: Db): Router {
  ...
  async function updateBalance(companyId: string, warehouseId: string, productId: string, delta: Decimal128, requireAvailable = false) {
    ...
    if (delta.toString().startsWith('-')) {
      ...
      filter.quantity = { $gte: amount } as StockBalance['quantity'] extends never ? never : Filter<StockBalance>['quantity'];
  ...
  router.get('/balances', async (req, res, next) => {
    try {
      ...
      const filter: Filter<StockBalance> = {};
  ...
  router.post('/movements', async (req, res, next) => {
    try {
      ...
      const product = await getProduct(a.companyId, input.productId);
      if (!product) { fail(res, 404, 'NOT_FOUND', 'Product not found'); return; }
      const warehouse = await getWarehouse(a.companyId, input.warehouseId);
      if (!warehouse) { fail(res, 404, 'NOT_FOUND', 'Warehouse not found'); return; }
      if (input.type === 'transfer') {
        const destination = await getWarehouse(a.companyId, input.destinationWarehouseId!);
        if (!destination) { fail(res, 404, 'NOT_FOUND', 'Destination warehouse not found'); return; }
        if (destination._id === warehouse._id) { fail(res, 400, 'VALIDATION_ERROR', 'Source and destination warehouses must differ'); return; }
        ...
        const destinationBalance = await updateBalance(a.companyId, input.destinationWarehouseId!, input.productId, amount);
        ...
        const now = new Date(); const movement: StockMovement = { _id: randomUUID(), companyId: a.companyId, type: 'transfer', productId: input.productId, warehouseId: input.warehouseId, destinationWarehouseId: input.destinationWarehouseId, quantity: amount, reason: input.reason, reference: input.reference, actorUserId: a.id, createdAt: now };
        await movements.insertOne(a.companyId, movement); await audit(a.companyId, a.id, 'stock.movement.transfer', 'stock_movement', movement._id!, { productId: input.productId, quantity: input.quantity, destinationWarehouseId: input.destinationWarehouseId });
      ...
      const delta = input.type === 'issue' || (input.type === 'adjustment' && input.direction === 'decrease') ? decimal(`-${input.quantity}`) : amount;
      const balance = await updateBalance(a.companyId, input.warehouseId, input.productId, delta, delta.toString().startsWith('-'));
      ...
      try { await movements.insertOne(a.companyId, movement); }
      catch (error) { await updateBalance(a.companyId, input.warehouseId, input.productId, decimal(delta.toString().startsWith('-') ? input.quantity : `-${input.quantity}`)); throw error; }
      await audit(a.companyId, a.id, `stock.movement.${input.type}`, 'stock_movement', movement._id!, { productId: input.productId, quantity: input.quantity, direction: input.direction });
  ...
  router.post('/reservations', async (req, res, next) => {
    try {
      ...
      try { await reservations.insertOne(a.companyId, reservation); }
      catch (error) { await balances.updateOne(a.companyId, { warehouseId: input.warehouseId, productId: input.productId, reservedQuantity: { $gte: amount } }, { $inc: { reservedQuantity: decimal(`-${input.quantity}`) }, $set: { updatedAt: new Date() } }); throw error; }
      await audit(a.companyId, a.id, 'stock.reservation.create', 'stock_reservation', reservation._id!, { productId: input.productId, quantity: input.quantity });
  ...
  router.delete('/reservations/:id', async (req, res, next) => {
    try {
      ...
      await reservations.updateOne(a.companyId, { _id: reservationId, active: true }, { $set: { active: false, updatedAt: new Date() } });
      await audit(a.companyId, a.id, 'stock.reservation.release', 'stock_reservation', reservationId);
      noContent(res);
    } catch (error) { next(error); }
  });
  ...
  router.get('/summary', async (req, res, next) => {
    try {
      if (!canRead(res)) { fail(res, 403, 'FORBIDDEN'); return; }
      const a = actor(res); const warehouseId = typeof req.query.warehouseId === 'string' ? id.parse(req.query.warehouseId) : undefined;
      const filter: Filter<StockBalance> = warehouseId ? { warehouseId } : {};
      const rows = await balances.find(a.companyId, filter).toArray();
      const totalQuantity = rows.reduce((sum, row) => sum + Number(row.quantity.toString()), 0);
      const totalReserved = rows.reduce((sum, row) => sum + Number(row.reservedQuantity.toString()), 0);
